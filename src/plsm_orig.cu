...
//dims_Pwtad(nDocs,1,1);
//thread 64 loop over nTs,
// sequential over tr,w to avoid concurrent write
	 ...
	 for (unsigned int nvoc_counter=0;nvoc_counter<iterations_nvoc;nvoc_counter++)
	     {
	        ...
	        shared_block[threadIdx.x]=0;
                ...
                __syncthreads();
                //if ( doc_index <((blockIdx.x*(nTd)+ntd_counter)*g_nVoc+g_nVoc))
		if ( nvoc_counter*block_width+threadIdx.x< g_nVoc)
                      { //printf("debug1\n");
          		//shared_block[threadIdx.x]=g_Doc[(blockIdx.x*(g_nTs+g_nTr)+ntd_counter)*(g_nVoc)+nvoc_counter*1024+threadIdx.x];
                     // printf("block id:: %d:: index :: %d \n",blockIdx.x,((blockIdx.x*(g_nTs+g_nTr)+ntd_counter)*(g_nVoc)+nvoc_counter*1024+threadIdx.x));
                    ...
                    // if(threadIdx.x==0)
                    //printf("block::%d::ntd_counter::%d sharedblock::%0.8f  logshared_block ::%0.8f \n",blockIdx.x,ntd_counter,shared_block[threadIdx.x],log(g_Pwtad[doc_index]+DBL_MIN));
                     __syncthreads();
                       }
                //printf("blockid ::%d:: %0.8f \n",blockIdx.x,shared_block[0]);
	            ...
	            if (threadIdx.x<s)
			{
                ...
                //printf("block::%d::ntd_counter::%d::logshared_block sum ::%0.8f::DBL_MIN is:: %0.8f \n",blockIdx.x,ntd_counter,shared_logmat_block[0],DBL_MIN);
                }
                __syncthreads();
	     }
...
//g_Logmat_sum[blockIdx.x]=logmat_sum/sum;  // matlab
...
__syncthreads();
			 ...
			 if(threadIdx.x==0)
			   {
				...
				//printf("tmp sum is ::%0.8f y is :: %d\n",tmp_sum, blockIdx.y);
		           	__syncthreads();
			   }
	            }
		 if (threadIdx.x==0 )
                     {
			//g_sum[blockIdx.y]=tmp_sum+(tmp_sum==0);
		...
		__syncthreads();
			 ...
			 {		      
			 // printf("g_sum[blockIdx.y] is ::%0.8f y is :: %d\n",g_sum[blockIdx.y], blockIdx.y);
...
cout<<"total_docs now is "<<total_docs<<endl;
	...
	while (f_count < src_doc.size()) {
		...
		doc.push_back(src_doc.at(f_count));
		//cout <<"n_line is:: "<<n_line << endl;

		/*

		if (d_overlap && m_numDocs>=1)
		{
		if ( overlap_ctr <= nTz-1)
		{
			for(std::vector<WordCount>::const_iterator it=src_doc.at(f_count).begin();it!=src_doc.at(f_count).end();++it )
		
		h_Doc[(docLength*(m_numDocs-1)+docLength-nTz+1+overlap_ctr)*m_corpusLength+it->first]=it->second;
		overlap_ctr++;
		}
		else
		{
		...
		h_Doc[(docLength*(m_numDocs)+n_line-overlap_ctr)*m_corpusLength+it->first]=it->second;
		...
		*/
		...
		//h_Doc[(docLength*total_docs+i)*m_corpusLength+it->first]=it->second;
		
		//cout <<"pair is first,second::"<< it->first<<","<<it->second	 <<endl;
		//for(int i=0;i<src_doc.at(f_count).size();i++ )
		//std::cout << "pair value is ::"<< src_doc.at(f_count)[i] <<endl;
		n_line++;
		f_count++;
		frame.clear();
		//cout<<f_count<<"\t"<<n_line<<"\t";
	...
	if (doc.size() > 0) {
	    ...
	    //data->add(doc);
            //data->m_docLength.push_back (docLength);
       // int i=0;
	//for(std::vector<WordCount>::const_iterator it=src_doc.at(f_count).begin();it!=src_doc.at(f_count).end();++it,++i )
	...
	//	h_Doc[(docLength*m_numDocs+n_line)*m_corpusLength+it->first]=it->second;
	    ...
	    //data->m_numDocs++;
	      m_numDocs++;
 cout <<"m_numDocs is:: "<<m_numDocs << endl;
	}
...
seed=1487059346; // temporary to test cpu result remove later
	...
	// initializations 
    	...
    	// loading from txt file
    	  ...
    	  if (trainFlag) 
	      {
				...
				//p_obj.random_initialize(h_Pzd,nZ,nDocs,1);
				...
				//p_obj.normalize(h_Ptszd,nDocs,nZ,nTs);
				...
				//p_obj.normalize(p_w_given_tr_z,nZ,nTr,nVoc);
				...
				//p_obj.print(log_trp,1,nTz+2,1);
				
				// read from files to check output
				    const char *datafile="input_";
				    const char *ext_ptszd="ptszd.txt";
				    const char *ext_trwz="trwz.txt";
				    const char *ext_zd="zd.txt";
				    const char *ext_pwz="pwz.txt";
                                    const char *ext_trp="trp.txt";	
				    //saveArray(const char* datafile, const char *ext, int n_Aspects, int n_Tz) 
				   // ptszd->saveArray(datafile,ext_ptszd,m_nAspects,m_nTz)	;
				   // ptrwz->saveArray(datafile,ext_trwz,m_nAspects,m_nTz)	;
				   // pzd->saveArray(datafile,ext_zd,m_nAspects,m_nTz)	;
				   // pwtad->saveArray(datafile,ext_pwtad,m_nAspects,m_nTz)	;

				

                               // p_obj.readArray( datafile, ext_ptszd,nZ, nTz, h_Ptszd,nZ,nDocs,nTs );
				//p_obj.file_initialize_tszd(ext_ptszd, h_Ptszd, nZ, nDocs, nTs);
				//p_obj.file_initialize_tszd( ext_trwz, h_Ptrwz,nVoc,nZ,nTz );
				//p_obj.file_initialize_tszd( ext_trp, trp,nZ,1,1 );
				//p_obj.file_initialize_tszd( ext_pwz, h_Pwz,nZ,nVoc,1 );
				//p_obj.readArray( datafile, ext_trwz,nZ, nTz, h_Ptrwz,nTz,nVoc,nZ );

				//p_obj.readArray( datafile, ext_trp,nZ, nTz, trp,nZ,nDocs,nTs );

				//p_obj.readArray( datafile, ext_pwz,nZ, nTz, h_Pwz,nZ,nVoc,1 );

				//p_obj.normalize_y(h_Ptszd,nDocs,nZ,nTs,3);
				//p_obj.normalize(h_Ptszd,nDocs,nZ,nTs);
				//p_obj.print(h_Ptszd,nZ,nTs,nDocs);
				//exit(0);
                                  printf("align_topics is %d\n",opt.align_topics);
				...
				//plsm_func(h_Doc, m_numDocs, docLength, m_corpusLength, n_Aspects, nTz);
  			      	}
			  if (initype == 2) // 
				{  // read pwz and ptszd for initializations
				 char *filename;
			         ...
			         //this->pwz->initialize(filename);
				//file_initialize(const char *filename, double *input, int ncols, int nrows, int ndepth)
				// p_obj.file_initialize(filename, h_Pwz, nZ, nVoc, 1);
                                // p_obj.file_initialize_3d(filename,h_Pwz, nZ, nVoc, 1);
				...
				//p_obj.file_initialize(filename, h_Ptszd, nZ, nDocs, nTs);
				p_obj.file_initialize_3d(filename, h_Ptszd, nZ, nDocs, nTs);
			        //this->ptszd->initialize(filename);
				...
				//p_obj.normalize(h_Ptszd,nDocs,nZ,nTs);
				//p_obj.normalize(h_Pwz,nVoc,nTz,1) ;
			  ...
			  if (initype == 3)

				{
				    ...
				    //that->pwz->initialize(filename);
				    //that->pwz->add(.1 / that->m_nVoc);
				    //that->priorPwz->initialize(filename);
				    //that->priorPwz->multiplyBy(priorScale / that->m_nTz);
				    //printf("scale %f %f \n", priorScale, (float) that->priorPwz->mat[0][0][0]);
		...
		//compute pz matrix by using trained pzd file
				   ...
				   h_TrainPd = (double *) malloc(mem_size_traind);
				    ...
				    //ptszd->initialize(1.0);
				   // pzd->initialize(1.0);
				   // pwtad->initialize(0.0);
				   // doc_lhood->initialize(0.0);                                    
	...
	cout<<"Paramters are nDocs: "<<nDocs<<"and max_nTd is: "<<max_nTd<<"nTz is: "<<nTz<<"nVoc is :" <<nVoc<<"nZ is "<<nZ<<"nTs is:"<<nTs<<endl;
				    ...
				    const char *ext_zd="zd.txt";
				    const char *ext_pwz="pwz.txt";
                               // p_obj.saveArray(datafile, (const char *)"ptrwz", n_Aspects, nTz, h_Ptrwz,nVoc,nZ,nTz);
	                      //  p_obj.file_initialize_3d(ext_ptszd, h_Ptszd, nZ, nDocs, nTs);
				//p_obj.file_initialize_3d( ext_trwz, h_Ptrwz,nVoc,nZ,nTz );
				//
				//p_obj.file_initialize_2d(ext_pwz, h_Pwz,nVoc,nZ);
				
			      
			    //  p_obj.file_initialize_2d( ext_zd, h_Pzd,nZ,nDocs );


			//p_obj.saveArray(datafile, (const char * )"pzd", n_Aspects, nTz, h_Pzd,nZ,nDocs,1 );
	               // p_obj.saveArray(datafile, (const char *)"ptszd", n_Aspects, nTz, h_Ptszd,nZ,nDocs,nTs ); 
        		//p_obj.saveArray(datafile, (const char *)"ptrwz", n_Aspects, nTz, h_Ptrwz,nVoc,nZ,nTz);
			//_obj.saveArray(datafile, (const char *)"pwz", n_Aspects, nTz, h_Pwz,nZ,nVoc,1 );
...
/*
     dim3 dims_Doc(nTd,nDocs,nVoc);
    unsigned int mem_size_Doc = dims_Doc.x * dims_Doc.y *dims_Doc.z* sizeof(double);
    double *h_Doc = (double *) malloc(mem_size_Doc);
   // memset(h_Doc,1,mem_size_Doc);
      ifstream fp_doc;
      fp_doc.open("doc_array.txt");

    for (int i=0;i<nDocs;i++)
            { for (int j=0;j<nTd;j++)
                  {for (int k=0;k<nVoc;k++)
                        {
                        fp_doc>>tmp;
                        h_Doc[(i*nTd+j)*nVoc+k]=tmp;
                      //  cout<<"index:"<<(i*nZ+j)*nTs+k<<"  Ptszd:: "<<h_Ptszd[(i*nZ+j)*nTs+k]<<endl;
                        }
                   }
             }
...
*/
    ...
    //vector<vector<vector<double> > >Ptszd(nTs, vector<vector<double> >(nZ, vector<double>(nDocs,1)));
    //x,y,z for dimensions
     // host variable initialisations 
    dim3 dims_Ptszd(nZ,nDocs,nTs);
    unsigned int mem_size_Ptszd = dims_Ptszd.x * dims_Ptszd.y *dims_Ptszd.z* sizeof(double);
    //double *h_Ptszd = (double *) malloc(mem_size_Ptszd);
    // doing mem set
    ...
    unsigned int mem_size_Ptrwz = dims_Ptrwz.x * dims_Ptrwz.y *dims_Ptrwz.z* sizeof(double);
    //double *h_Ptrwz = (double *) malloc(mem_size_Ptrwz);
    //memset(h_Ptrwz,1,mem_size_Ptrwz);
   ...
   // double *h_mat_Pwtad = (double *) malloc(mem_size_Pwtad);
    ...
    //output variable
    //dim3 dims_tszd(nZ, nTs, nDocs);
    //dim3 dims_tszd(nZ, nDocs, nTs);
    //unsigned int mem_size_tszd = dims_tszd.x * dims_tszd.y *dims_tszd.z* sizeof(double);
    double *h_tszd = (double *) malloc(mem_size_Ptszd);
    //memset(h_tszd,0,mem_size_tszd);


    //dim3 dims_zd(nZ,nDocs,1);
    //unsigned int mem_size_zd = dims_zd.x * dims_zd.y *dims_zd.z* sizeof(double);
    double *h_zd = (double *) malloc(mem_size_Pzd);
    //memset(h_zd,0,mem_size_zd);
...
//double *d_wz_norm;
//checkCudaErrors(cudaMalloc((void **) &d_wz_norm, mem_size_Pwz));
...
//double *d_trwz_norm;
//checkCudaErrors(cudaMalloc((void **) &d_trwz_norm, mem_size_Ptrwz));


double *h_zd_norm = (double *) malloc(mem_size_Pzd);
//double *d_zd_norm;
//checkCudaErrors(cudaMalloc((void **) &d_zd_norm, mem_size_Pzd));


double *h_tszd_norm = (double *) malloc(mem_size_Ptszd);
//double *d_tszd_norm;
//checkCudaErrors(cudaMalloc((void **) &d_tszd_norm, mem_size_Ptszd));
...
/*
double *h_Docsum = (double *) malloc(mem_size_logmatsum);
double *d_Docsum;
checkCudaErrors(cudaMalloc((void **) &d_Docsum, mem_size_logmatsum));

calc_sum_doc<<<grid_sumdoc,threads_sumdoc>>>(d_Docsum,d_Doc,nDocs,nTs,nVoc, nTr,totWords);
cudaDeviceSynchronize();
checkCudaErrors(cudaMemcpy(h_Docsum, d_Docsum, mem_size_logmatsum,cudaMemcpyDeviceToHost));
//for (int r = 0; r < nDocs; r++) 
//printf("r is %d, sum is %lf \n",r,h_Docsum[r]);
*/
...
//
//grid  nDocs
//thread 16 loop nz 32 loop over nTs,
//dims_Pwtad(nVoc,nTr,nDocs);
//thread 64 loop over nTs,
//dim3 grid_pwtad(nVoc,nTr,nDocs);
...
dim3  grid_1(1, 1,1);
dim3  block_1(1,1,1);
// kernelcalculate observed likelihood
//calc_observ_lhood<<<grid_1,block_1>>>(L_prev,d_Logmatsum,nDocs);
//cudaDeviceSynchronize();
//checkCudaErrors(cudaMemcpy(h_Logmatsum, d_Logmatsum, mem_size_logmatsum,cudaMemcpyDeviceToHost));

// or sequentially perform sum 
for (int i=0;i<nDocs;i++)
...
//for (int r = 0; r < 10; r++) //nVoc*nTd*nDocs
//printf("pwtad value ::  %0.8f \n",h_Pwtad[r]);
...
//for (int r = 0; r < nZ*nTs*nDocs; r++) 
//printf("ptszd value ::  %lg \n",h_Ptszd[r]);
...
//dim3  grid_tszd(1, 1,1);
   ...
   //calc_tszd(double *g_zd, double *g_tszd,double *g_Ptszd,double *g_Pzd,double *g_Pwz,double *g_Ptrwz,double *g_Pwtad,double *g_Doc,double *pd,double *pz, int g_nDocs,int g_nZ, int g_nTs,int g_nVoc, int g_nTr,float lambdaTsSparsity, bool z_prior , bool m_trainflag)
...
//checkCudaErrors(cudaMemcpy(h_zd, d_zd, mem_size_zd,cudaMemcpyDeviceToHost));
//for (int i=0;i<10;i++)
//{
//printf(" i is %d and tszd value is %0.8f\n",i,h_tszd[i]);
//exit(0);
//normalize
...
/*
if (opt.lambdaTsSparsity!=0)
{
// tszd x nz y d and z is ts
dim3  grid_sparsity(nZ,nDocs,1);
dim3  threads_sparsity(32,1, 1);
tszd_sparsityflag<<<grid_sparsity,threads_sparsity>>>(d_tszd,d_Docsum,lambda,nDocs,nZ,nTs,nVoc);
cudaDeviceSynchronize();
checkCudaErrors(cudaMemcpy(h_tszd, d_tszd, mem_size_tszd,cudaMemcpyDeviceToHost));
}
*/
...
//__global__ void calc_trwz(double *g_trwz,double *g_wz,double *g_Ptszd,double *g_Pzd,double *g_Pwz,double *g_Ptrwz,double *g_Pwtad,double *g_Doc,double *pd,int g_nDocs,int g_nZ, int g_nTs,int g_nVoc, int g_nTr,bool usePriorPtrwz,bool tr_prior)
dim3  grid_trwz(nVoc,nZ,1);
dim3  threads_trwz(64, 1,1);
// need to implement m_Options.fitMode == NORMAL || m_Options.fitMode == NORMAL_UNIFORM) currently working for default mode

//calc_trwz<<<grid_trwz,threads_trwz>>>(d_trwz,d_wz,d_tszd_norm,d_zd_norm,d_Pwz,d_Ptrwz,d_Pwtad,d_Doc,d_Pd,nDocs,nZ,nTs,nVoc,nTr,opt.tr_prior,opt.tr_prior);
...
//dim3  grid_wz(nVoc,nZ,1);
//dim3 threads_wz(32,16);
//(nts, nTr,1);

//__global__ void calc_wz(double *g_wz,double *g_Ptszd,double *g_Pzd,double *g_Pwz,double *g_Ptrwz,double *g_Pwtad,double *g_Doc,int g_nDocs,int g_nZ, int g_nTs,int g_nVoc, int g_nTr)
  
//calc_wz<<<grid_wz,threads_wz>>>(d_wz,d_tszd_norm,d_zd_norm,d_Pwz,d_Ptrwz,d_Pwtad,d_Doc, nDocs,nZ, nTs,nVoc, nTr);
//cudaDeviceSynchronize();
//checkCudaErrors(cudaMemcpy(h_wz, d_wz, mem_size_Pwz,cudaMemcpyDeviceToHost));
...
//dim3  grid(nVoc,nZ,nTr);
    //dim3  threads(64)(nts, 1,1);
// loop over ndocs
//dim3  grid_trwz(nVoc,nZ,1);
//dim3  threads_trwz(64, 1,1);
//__global__ void calc_trwz(double *g_trwz,double *g_Ptszd,double *g_Pzd,double *g_Pwz,double *g_Ptrwz,double *g_Pwtad,double *g_Doc,int g_nDocs,int g_nZ, int g_nTs,int g_nVoc, int g_nTr)
//calc_trwz<<<grid_trwz,threads_trwz>>>(d_trwz,d_tszd_norm,d_zd_norm,d_wz_norm,d_Ptrwz,d_Pwtad,d_Doc,nDocs,nZ,nTs,nVoc,nTr);
//checkCudaErrors(cudaMemcpy(h_trwz,d_trwz, mem_size_Ptrwz,cudaMemcpyDeviceToHost));
...
//for (int i=0;i<nTs*nDocs*nZ;i++)
//printf("pwtad_new:: h_Ptszd[%d]:: %0.8f \n",i,h_Ptszd[i]);
//1024(nvoc) block size and number of blocks is nDocs 
//__global__ void calc_logmatsum(double *g_Logmat_sum,double *g_Docsum, double *g_Doc,double *g_Pwtad,int g_nDocs, int g_nTs,int g_nVoc, int g_nTr)
...
//for (int i=0;i<nDocs;i++)
//printf("h_logmat old  h_logmatsumnew for id %d is %0.8f:: %0.8f\n",i,h_Logmatsum[i],h_Logmatsum_new[i]);
...
//dim3  block_2(1,1,1);
// kernelcalculate observed likelihood
//calc_observ_lhood<<<grid_2,block_2>>>(d_L,d_Logmatsum_new,nDocs);
//checkCudaErrors(cudaMemcpy(L, d_L, mem_size_1,cudaMemcpyDeviceToHost));
//cudaDeviceSynchronize();
for (int i=0;i<nDocs;i++)
...
if ( ( iterations>5 && (abs(L_Diff) < end_accuracy) ) || iterations >= Max_iterations-1)
	...
	//printf("L_plsm is %0.8f\n",L_plsm);
	//printf("h_ptszd 0 is %lg \n",h_Ptszd[0]);
	//printf("h_ptszd 1 is %lg \n",h_Ptszd[1]);
	//printf("h_tszd_norm 0 is %lg \n",h_tszd_norm[0]);
	//printf("h_tszd_norm 1 is %lg \n",h_tszd_norm[1]);
	//h_Ptszd=h_tszd_norm;
	//h_Pzd=h_zd_norm;
	//h_Ptrwz=h_trwz_norm;
	//h_Pwz=h_wz_norm;
	...
	//for (int i=0;i<nZ*nVoc*nTr;i++)
	//printf("h_trwz_norm is:: %0.8f\n",h_trwz_norm[i]);
	//dim3  grid(nTr,nZ,1);
	    //dim3  threads(16)(Nvoc, 1,1);
	// loop over ndocs
	
	//dim3  grid_joint(nTr,nZ,1);
	//dim3  threads_joint(16,1,1);
	//MakeJoint(double *g_Topics,double *g_Pwz,double *g_Ptrwz,int g_nZ,int g_nVoc, int g_nTr)
	// MakeJoint<<<grid_joint,threads_joint>>>(double *g_Topics,double *g_Pwz,double *g_Ptrwz,int g_nZ,int g_nVoc, int g_nTr)
	}
